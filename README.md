...
- **多维度搜索引擎** — OpenSearch 全文检索，支持文章/用户/图片聚合搜索与关键词高亮
...
| 功能 | 描述 |
|:-----|:-----|
| 综合搜索 | 聚合搜索文章/用户/图片，按相关性加权混合排序 |
| 文章搜索 | 标题/正文/标签全文检索，分类/时间/热度多维筛选，关键词高亮 |
| 用户搜索 | 按用户名/昵称/拼音搜索，已关注用户优先展示 |
| 图片搜索 | 搜索文章封面图及内嵌图，点击跳转原文 |
| 搜索建议 | 输入时实时补全建议（用户 + 文章 + 标签） |
...
| 内容管理 | 文章/分类/标签/评论增删改查、置顶、推荐 |
| 用户管理 | 用户查询、封禁/解封、角色分配 |
| 数据看板 | 用户总量、文章总量、日活、浏览量、互动量及趋势分析 |
...
| 表名 | 说明 |
|:-----|:-----|
| `sys_user` | 用户表（在线状态、关注/粉丝/文章计数冗余） |
| `cms_article` | 文章表（热度分、软删除） |
| `cms_category` | 分类表（树形结构） |
| `cms_tag` | 标签表 |
| `cms_article_tag` | 文章-标签关联表 |
| `cms_article_image` | 文章图片表（封面图/内嵌图） |
...
| 阶段 | 目标 | 核心交付 |
|:-----|:-----|:---------|
| **P0 — MVP** | 最小可用系统 | 注册登录、文章发布审核、分类管理、点赞收藏、关注私信、个性化推荐、热点排行榜、基础搜索、数据看板、基础安全防护 |
