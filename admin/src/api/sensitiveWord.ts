...
export function getSensitiveWordList(params?: PageParams & { keyword?: string; level?: SensitiveWordLevel }) {
  return get<PageResult<SensitiveWord>>('/admin/sensitive-words', params as unknown as Record<string, unknown>)
}

/** 添加敏感词 */
export function createSensitiveWord(data: { word: string; level: SensitiveWordLevel }) {
  return post<SensitiveWord>('/admin/sensitive-words', data as unknown as Record<string, unknown>)
}

/** 批量添加敏感词 */
export function batchCreateSensitiveWords(data: { words: string[]; level: SensitiveWordLevel }) {
  return post('/admin/sensitive-words/batch', data as unknown as Record<string, unknown>)
}
...
export function updateSensitiveWordLevel(id: number, level: SensitiveWordLevel) {
  return post(`/admin/sensitive-words/${id}/level`, { level } as unknown as Record<string, unknown>)
}
