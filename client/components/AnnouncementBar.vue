<template>
  <div v-if="announcements.length > 0" class="announcement-bar">
    <div
      class="flex items-center gap-3 px-4 py-2.5 rounded-lg bg-blue-50 border border-blue-100"
    >
      <!-- 图标 -->
      <div class="shrink-0">
      ...
      <div class="flex-1 overflow-hidden relative h-5">
        <TransitionGroup name="announcement-slide">
          ...
          >
            <span
              class="text-sm text-blue-700 truncate cursor-pointer hover:underline"
              @click="handleClick(item)"
            >
              <span
                v-if="item.type === 2"
                class="inline-flex items-center mr-1.5 px-1.5 py-0.5 text-xs font-medium rounded bg-amber-100 text-amber-700"
              >活动</span>
              <span
                v-else
                class="inline-flex items-center mr-1.5 px-1.5 py-0.5 text-xs font-medium rounded bg-blue-100 text-blue-700"
              >公告</span>
              {{ item.title }}
            </span>
          </div>
      ...
      <button
        v-if="currentItem"
        class="shrink-0 text-xs text-blue-500 hover:text-blue-700"
    ...
    <Transition name="announcement-detail">
      <div
        ...
        class="mt-2 px-4 py-3 rounded-lg bg-blue-50/50 border border-blue-100"
      >
        <h4 class="font-medium text-blue-800 mb-1">{{ currentItem.title }}</h4>
        <p class="text-sm text-slate-600 whitespace-pre-wrap">{{ currentItem.content }}</p>
      </div>
    </Transition>
  </div>
</template>
