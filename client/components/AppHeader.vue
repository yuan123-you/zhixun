<template>
  <!-- 顶部导航栏 -->
  <header class="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-lg border-b border-slate-200/60 shadow-[var(--shadow-sm)]">
    <div class="max-w-[1200px] 2xl:max-w-[1400px] mx-auto px-2 2xl:px-6 h-12 md:h-16 flex items-center justify-between gap-2">
      ...
      <div class="flex items-center gap-2 shrink-0">
        <!-- 返回按钮（非首页时显示） -->
        <button v-if="showBackButton" class="p-1.5 text-slate-600 hover:bg-slate-50 rounded-lg" @click="goBack">
        ...
        <!-- 移动端汉堡菜单按钮（首页时显示） -->
        <button v-if="!showBackButton" class="md:hidden p-1.5 text-slate-600 hover:bg-slate-50 rounded-lg" @click="showMobileMenu = !showMobileMenu">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="flex items-center space-x-1 md:space-x-3">
        <!-- 搜索图标 -->
        <NuxtLink to="/search" class="p-1.5 md:p-2 text-slate-600 hover:bg-slate-50 rounded-lg transition-colors" title="搜索">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- 消息中心图标（已登录） -->
        <ClientOnly>
          <NuxtLink v-if="userStore.isLoggedIn" to="/notifications" class="relative p-1.5 md:p-2 text-slate-600 hover:bg-slate-50 rounded-lg transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
