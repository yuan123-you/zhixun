<template>
  <!-- 文章卡片组件 - 微博风格布局 -->
  <article class="card px-3 py-4 md:px-4 md:py-5 hover:shadow-md transition-shadow cursor-pointer no-tap-highlight touch-feedback" @click="navigateToDetail">
    <!-- 作者信息栏（标题上方） -->
    ...
    <div class="flex items-center mt-1 -ml-1">
      <!-- 点赞 -->
      <button
        class="flex items-center gap-1 px-2.5 py-2 rounded-full text-sm transition-colors touch-target"
      ...
      <!-- 评论 -->
      <button
        class="flex items-center gap-1 px-2.5 py-2 rounded-full text-sm text-gray-400 dark:text-gray-500 hover:text-primary transition-colors touch-target"
