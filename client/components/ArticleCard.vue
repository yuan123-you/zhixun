<template>
  <!-- 文章卡片组件 - 微博风格布局 -->
  <article class="card px-4 py-5 md:px-5 md:py-6 hover:shadow-md transition-shadow cursor-pointer no-tap-highlight touch-feedback" @click="navigateToDetail">
    <!-- 作者信息栏（标题上方） -->
    <div class="flex items-center gap-2.5 mb-2">
      ...
      <div class="flex-1 min-w-0 flex items-center">
        ...
        <span class="text-xs text-gray-400 dark:text-gray-500 mx-1.5 shrink-0">·</span>
        <time class="text-xs text-gray-400 dark:text-gray-500 shrink-0">{{ formatTimestamp(article.createdAt) }}</time>
        <span v-if="article.deviceInfo" class="text-xs text-gray-400 dark:text-gray-500 ml-1.5 shrink-0">来自{{ article.deviceInfo }}</span>
        <span v-if="article.matchType" class="ml-2 inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium shrink-0"
    ...
    <div class="flex items-center mt-2 gap-1">
      <!-- 点赞 -->
      <button
        class="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm transition-colors hover:bg-gray-100 dark:hover:bg-gray-700 active:scale-95"
        :class="article.isLiked ? 'text-primary' : 'text-gray-400 dark:text-gray-500 hover:text-primary'"
        @click.stop="handleToggleLike"
      >
        <svg class="w-[18px] h-[18px]" :class="article.isLiked ? 'fill-primary' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <!-- 评论 -->
      <button
        class="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm text-gray-400 dark:text-gray-500 hover:text-primary hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors active:scale-95"
