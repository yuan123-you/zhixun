<template>
  ...
  <div>
    ...
    <!-- 多级子回复（递归渲染，支持展开/收起） -->
    <div v-if="hasReplies" class="relative ml-4 pl-4 border-l-2 border-slate-200">
      <!-- 展开/收起按钮 -->
      <button
        v-if="collapsible"
        class="flex items-center space-x-1 text-xs text-primary hover:text-primary/80 transition-colors mt-2 -ml-[calc(1rem+2px)]"
      ...
      >
        ...
        <span>{{ expanded ? '收起回复' : `${totalReplyCount}条回复` }}</span>
      </button>

      <!-- 子回复列表 -->
      <div v-show="expanded" class="space-y-3 pt-2">
        <CommentItem
