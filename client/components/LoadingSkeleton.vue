  ...
  <div class="animate-pulse">
    ...
    <template v-if="type === 'article'">
      <div class="card p-4">
        <div class="flex gap-4">
          ...
          <!-- 封面图 -->
          <div class="hidden sm:block w-32 h-24 bg-gray-200 dark:bg-gray-700 rounded-lg shrink-0"></div>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="type === 'user'">
      <div class="card p-4 flex items-center space-x-3">
        <div class="w-12 h-12 bg-gray-200 dark:bg-gray-700 rounded-full shrink-0"></div>
...
withDefaults(defineProps<{
  type?: 'article' | 'user' | 'comment' | 'default'
}>(), {
  type: 'default',
})
</script>
