<template>
  ...
  <nav class="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 safe-bottom no-tap-highlight">
    <div class="flex items-center justify-around h-14">
      ...
      <!-- 热门 -->
      <NuxtLink to="/rank" class="flex flex-col items-center justify-center flex-1 py-1" :class="isActive('/rank') ? 'text-primary' : 'text-gray-500 dark:text-gray-400'">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 18.657A8 8 0 016.343 7.343S7 9 9 10c0-2 .5-5 2.986-7C14 5 16.09 5.777 17.656 7.343A7.975 7.975 0 0120 13a7.975 7.975 0 01-2.343 5.657z" />
        </svg>
        <span class="text-2xs mt-0.5">热门</span>
      </NuxtLink>
      ...
      <!-- 消息 -->
      <NuxtLink to="/messages" class="flex flex-col items-center justify-center flex-1 py-1 relative" :class="isActive('/messages') ? 'text-primary' : 'text-gray-500 dark:text-gray-400'">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <!-- 通知 -->
      <NuxtLink to="/notifications" class="flex flex-col items-center justify-center flex-1 py-1 relative" :class="isActive('/notifications') ? 'text-primary' : 'text-gray-500 dark:text-gray-400'">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
        </svg>
        <span class="text-2xs mt-0.5">通知</span>
        <!-- 未读通知红点 -->
