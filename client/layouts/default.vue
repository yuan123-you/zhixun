<template>
  ...
  <div class="min-h-screen bg-[var(--color-bg)]">
    ...
    <ClientOnly>
      <aside
        v-if="isTablet"
        class="tablet-sidebar fixed left-0 top-12 md:top-16 bottom-0 w-[260px] z-40 bg-white border-r border-slate-200 overflow-y-auto transition-transform duration-300"
        :class="isTabletSidebarOpen ? 'translate-x-0' : '-translate-x-full'"
      >
        <div class="p-3 space-y-3">
          <!-- 侧边栏导航 -->
          <nav class="space-y-1">
            <NuxtLink to="/" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors no-tap-highlight touch-target" :class="isActive('/') ? 'bg-primary-50 text-primary-700' : 'text-slate-700 hover:bg-slate-50'" @click="closeTabletSidebar">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
              </svg>
              <span>首页</span>
            </NuxtLink>
            <NuxtLink to="/rank" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors no-tap-highlight touch-target" :class="isActive('/rank') ? 'bg-primary-50 text-primary-700' : 'text-slate-700 hover:bg-slate-50'" @click="closeTabletSidebar">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 18.657A8 8 0 016.343 7.343S7 9 9 10c0-2 .5-5 2.986-7C14 5 16.09 5.777 17.656 7.343A7.975 7.975 0 0120 13a7.975 7.975 0 01-2.343 5.657z" />
              </svg>
              <span>排行</span>
            </NuxtLink>
            <NuxtLink v-if="userStore.isLoggedIn" to="/editor" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors no-tap-highlight touch-target" :class="isActive('/editor') ? 'bg-primary-50 text-primary-700' : 'text-slate-700 hover:bg-slate-50'" @click="closeTabletSidebar">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="border-t border-slate-200 pt-3">
            <h3 class="px-3 text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">分类</h3>
            <div class="space-y-1">
              <NuxtLink to="/category/tech" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-slate-600 no-tap-highlight touch-target" @click="closeTabletSidebar">
                <span class="w-2 h-2 rounded-full bg-blue-500"></span>
                <span>技术</span>
              </NuxtLink>
              <NuxtLink to="/category/design" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-slate-600 no-tap-highlight touch-target" @click="closeTabletSidebar">
                <span class="w-2 h-2 rounded-full bg-purple-500"></span>
                <span>设计</span>
              </NuxtLink>
              <NuxtLink to="/category/product" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-slate-600 no-tap-highlight touch-target" @click="closeTabletSidebar">
          ...
          <!-- 热门标签 -->
          <div class="border-t border-slate-200 pt-3">
            <h3 class="px-3 text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">热门标签</h3>
