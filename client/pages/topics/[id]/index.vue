<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    <div class="max-w-4xl mx-auto px-4 py-6">
      <div v-if="topic" class="bg-white dark:bg-gray-800 rounded-2xl p-6 mb-6 shadow-sm">
        <div class="flex items-start gap-4">
          <div v-if="topic.coverImage" class="w-20 h-20 rounded-xl overflow-hidden flex-shrink-0">
            <img :src="topic.coverImage" class="w-full h-full object-cover" />
          </div>
          <div v-else class="w-20 h-20 rounded-xl bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center text-white font-bold text-2xl flex-shrink-0">
            {{ topic.name.charAt(0) }}
          </div>
          <div class="flex-1 min-w-0">
            ...
            <p class="text-gray-500 dark:text-gray-400 text-sm mb-3">{{ topic.description || '暂无描述' }}</p>
          ...
          <button @click="toggleFollow"
            ...
            class="px-5 py-2 rounded-full text-sm font-medium transition flex-shrink-0">
