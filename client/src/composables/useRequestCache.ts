...
export function useRequestCache(defaultOptions: RequestCacheOptions = {}) {
   ...
   * @param url 请求 URL
   * @param params 请求参数
   * @param options 缓存选项（可传 { force: true } 忽略 stale 缓存的污染）
   */
  const cachedRequest = async <T>(
    requestFn: () => Promise<T>,
    url: string,
    params?: Record<string, any>,
    options?: RequestCacheOptions & { force?: boolean }
  ): Promise<T> => {
    ...
    // 2. SWR 策略：有过期内存缓存时先返回旧数据，后台刷新（force 模式跳过）
    if (swr && !force) {
    ...
    // 3. 内存缓存未命中，尝试 localStorage 持久化缓存（force 模式跳过）
    if (persist && !force) {
