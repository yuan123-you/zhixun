<template>
  ...
  <div class="max-w-[1200px] 2xl:max-w-[1400px] mx-auto px-1.5 2xl:px-2 py-1.5">
    <div class="flex items-center justify-between mb-1.5">
      <p class="text-[var(--zh-text-secondary)] dark:text-[var(--zh-text-tertiary)]">{{ isOwn ? '你' : userName }}的粉丝</p>
    ...
    <div v-else class="space-y-2">
      <div v-for="user in users" :key="user.id" class="card p-3 flex items-center justify-between hover:shadow-md transition-shadow">
        <RouterLink :to="`/user/${user.id}`" class="flex items-center gap-3 min-w-0 flex-1">
          ...
          <div class="min-w-0">
            <p class="font-medium text-[var(--zh-text)] truncate">{{ user.nickname }}<span v-if="user.isMutualFollow" class="text-xs text-primary ml-1">互关</span></p>
            <p class="text-xs text-[var(--zh-text-tertiary)] truncate">{{ user.bio || '这个人很懒，什么都没写' }}</p>
          </div>
        </RouterLink>
