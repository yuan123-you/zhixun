<template>
  ...
  <div class="fixed inset-0 z-[200] bg-black flex flex-col">
    ...
    <div class="absolute top-0 left-0 right-0 z-30 h-12 flex items-center px-2 bg-gradient-to-b from-black/60 to-transparent pointer-events-none">
      <button
        class="pointer-events-auto w-9 h-9 flex items-center justify-center rounded-full bg-[var(--zh-bg-elevated)]/10 backdrop-blur-sm text-white active:bg-[var(--zh-bg-elevated)]/20 transition-colors"
        @click="goBack"
      >
    ...
    >
      ...
      <div v-else-if="error" class="flex flex-col items-center justify-center h-full text-white/70 px-6">
        ...
        <button class="px-4 py-2 bg-[var(--zh-bg-elevated)]/10 rounded-lg text-sm active:bg-[var(--zh-bg-elevated)]/20 transition-colors" @click="fetchArticle">
          重新加载
        </button>
      </div>
      ...
      <div v-else-if="article" class="min-h-full">
        ...
        <div v-if="article.tags?.length" class="px-4 pb-4 flex flex-wrap gap-1.5">
          <span
            ...
            :key="tag.id"
            class="px-2.5 py-1 text-xs rounded-full bg-[var(--zh-bg-elevated)]/10 text-white/70"
          >
            #{{ tag.name }}
          </span>
        </div>
    ...
    <div class="absolute bottom-0 left-0 right-0 z-30 bg-black/90 backdrop-blur-lg border-t border-white/10">
      <div class="flex items-center justify-between px-4 py-2.5">
        ...
        <!-- 修改权限按钮 -->
        <button
          class="flex items-center gap-1 px-3 py-1.5 rounded-full bg-[var(--zh-bg-elevated)]/10 text-xs text-white/70 active:bg-[var(--zh-bg-elevated)]/20 transition-colors"
    ...
    <Teleport to="body">
        ...
        <div v-if="activeSheet" class="fixed inset-0 z-[250] flex flex-col justify-end">
          ...
          <div class="relative bg-gray-900 rounded-t-2xl w-full max-h-[75vh] overflow-hidden flex flex-col">
            ...
            <div class="flex justify-center pt-3 pb-1">
              <div class="w-8 h-1 rounded-full bg-[var(--zh-bg-elevated)]/20" />
            </div>
            ...
            <template v-if="activeSheet === 'menu'">
              <div class="px-4 py-2">
                ...
                <div class="grid grid-cols-3 gap-3">
                  <!-- 分享 -->
                  <button class="flex flex-col items-center gap-2 p-3 rounded-xl active:bg-[var(--zh-bg-elevated)]/5 transition-colors" @click="openShareSheet">
                  ...
                  <!-- 转发 -->
                  <button class="flex flex-col items-center gap-2 p-3 rounded-xl active:bg-[var(--zh-bg-elevated)]/5 transition-colors" @click="openForwardSheet">
                  ...
                  <!-- 删除 -->
                  <button class="flex flex-col items-center gap-2 p-3 rounded-xl active:bg-[var(--zh-bg-elevated)]/5 transition-colors" @click="confirmDeleteArticle">
              ...
              <div class="px-4 pb-6 pt-2">
                <button class="w-full py-3 rounded-xl bg-[var(--zh-bg-elevated)]/5 text-sm text-white/70 active:bg-[var(--zh-bg-elevated)]/10 transition-colors" @click="closeAllSheets">
                  取消
                </button>
              </div>
            </template>
              ...
              <div class="px-4 py-2 flex-shrink-0">
                ...
                <div class="relative mb-3">
                  ...
                  <input
                    ...
                    type="text"
                    placeholder="搜索好友..."
                    class="w-full h-10 pl-9 pr-3 rounded-lg bg-[var(--zh-bg-elevated)]/5 text-sm text-white placeholder:text-white/30 border border-white/10 focus:border-white/30 focus:outline-none"
                  />
                </div>
              </div>
              <!-- 好友列表 -->
              <div class="flex-1 overflow-y-auto overscroll-contain px-4 pb-4">
                ...
                <div v-else class="space-y-1">
                  <button
                    ...
                    :key="user.id"
                    class="flex items-center gap-3 w-full p-2.5 rounded-lg active:bg-[var(--zh-bg-elevated)]/5 transition-colors"
            ...
            <template v-if="activeSheet === 'forward'">
              ...
              <div class="px-4 pb-6 space-y-1">
                <!-- QQ -->
                <button class="flex items-center gap-4 w-full p-3 rounded-xl active:bg-[var(--zh-bg-elevated)]/5 transition-colors" @click="forwardTo('qq')">
                ...
                <!-- 微信 -->
                <button class="flex items-center gap-4 w-full p-3 rounded-xl active:bg-[var(--zh-bg-elevated)]/5 transition-colors" @click="forwardTo('wechat')">
                ...
                <!-- 微博 -->
                <button class="flex items-center gap-4 w-full p-3 rounded-xl active:bg-[var(--zh-bg-elevated)]/5 transition-colors" @click="forwardTo('weibo')">
                ...
                <!-- 复制链接 -->
                <button class="flex items-center gap-4 w-full p-3 rounded-xl active:bg-[var(--zh-bg-elevated)]/5 transition-colors" @click="forwardTo('link')">
                  <div class="w-10 h-10 rounded-lg bg-[var(--zh-bg-elevated)]/10 flex items-center justify-center shrink-0">
              ...
              <div class="px-4 py-2">
                ...
                <div class="space-y-1">
                  <button
                    ...
                    :key="opt.value"
                    class="flex items-center gap-3 w-full p-3 rounded-xl active:bg-[var(--zh-bg-elevated)]/5 transition-colors"
                    @click="updateVisibility(opt.value)"
                  >
                    <div class="w-10 h-10 rounded-lg flex items-center justify-center shrink-0"
                      :class="article?.visibility === opt.value ? 'bg-blue-500/20' : 'bg-[var(--zh-bg-elevated)]/5'"
                    >
            ...
            <template v-if="activeSheet === 'views'">
              <div class="px-4 py-2 flex-shrink-0">
                ...
                <!-- Tab 切换 -->
                <div class="flex rounded-lg bg-[var(--zh-bg-elevated)]/5 p-0.5 mb-3">
                  <button
                    class="flex-1 py-2 text-xs font-medium rounded-md transition-colors"
                    :class="viewsTab === 'viewers' ? 'bg-[var(--zh-bg-elevated)]/10 text-white' : 'text-white/50'"
                  ...
                  <button
                    class="flex-1 py-2 text-xs font-medium rounded-md transition-colors"
                    :class="viewsTab === 'likers' ? 'bg-[var(--zh-bg-elevated)]/10 text-white' : 'text-white/50'"
        ...
        <div v-if="showDeleteConfirm" class="fixed inset-0 z-[260] flex items-center justify-center">
          ...
          <div class="relative bg-gray-900 rounded-2xl w-[320px] max-w-[88vw] p-6">
            ...
            <div class="flex gap-3">
              <button
                class="flex-1 py-2.5 rounded-xl text-sm font-medium text-white/70 bg-[var(--zh-bg-elevated)]/5 active:bg-[var(--zh-bg-elevated)]/10 transition-colors"
      ...
      <Transition name="sheet-slide">
        ...
        >
          <button class="absolute top-4 right-4 w-10 h-10 flex items-center justify-center rounded-full bg-[var(--zh-bg-elevated)]/10 text-white/70 active:bg-[var(--zh-bg-elevated)]/20 z-10" @click="imageZoomVisible = false">
...
// 核心修复：动态路由 articleId 变化时（切换预览作品）重置状态并重新加载
