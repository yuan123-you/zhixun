...
Get-Content $envFile -Encoding UTF8 | ForEach-Object {
    ...
    if ($line -and -not $line.StartsWith("#") -and $line -match "^\s*([A-Z_][A-Z0-9_]*)\s*=\s*(.*)$") {
        ...
        if ($name -notmatch "^(CLIENT_PORT|ADMIN_PORT|NGINX_HTTP_PORT|NGINX_HTTPS_PORT|SERVER_2_PORT|SONARQUBE_PORT|RABBITMQ_MGMT_PORT|OPENSEARCH_HTTP_PORT|OPENSEARCH_TRANSPORT_PORT|MINIO_API_PORT|MINIO_CONSOLE_PORT|MYSQL_PORT|REDIS_PORT|API_BASE|NUXT_PUBLIC_WS_BASE)$") {
